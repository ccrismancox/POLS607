...
relik <- function(x0, y, X, Ti, Xbar, ybar){
  ## Negative log-likelihood for the linear random effects model
  ## inputs: 
  ...
  ln2.detSIGMA.i <- 1/2 * log(Ti * s2.a + s2.e) + (Ti-1)/2 * log(s2.e) ## length N
  ...
  ll <- sum(ln2.detSIGMA.i) +  crossprod(e.tilde)/2 ## negative likelihood
  return(ll)
}
...
re_mle_gr <- function(x0, y, X, Ti,  Xbar, ybar){
  ## Gradient of the negative log-likelihood  for the linear random effects model
  ## inputs: 
  ##   x0: length-(k+2) matrix of parameter vector (theta, ln(sigma^2_alpha), ln(sigma^2_epsilon))
  ##   y: length-sum(Ti) vector of the outcome variable
  ##   X: sum(Ti) by k matrix of independent variables
  ...
  sigma2 <- Ti*s2.a+s2.e ## simplification for a common term
  w <- 1 - sqrt(s2.e)/sqrt(Ti * s2.a + s2.e)
  w <- rep(w, Ti)
  ehat  <-  drop(y-X %*%theta)
  ebar <- drop(ybar-Xbar %*%theta)
  ...
  ## Long piece of Ds2e that I don't want to mess up the 
  ## parentheses on
  inner <- ((sqrt(s2.e)/(rep(sigma2,Ti)^(3/2)))-
              (1/(sqrt(s2.e)*rep(sqrt(sigma2),Ti))))
  ...
  Ds2e <- sum((Ti-1)/(s2.e)) + 
    sum(1/(sigma2)) - 
    sum(((ehat-ebar*w)^2)/(s2.e^2)) -
    sum( (ebar*(ehat-ebar*w)*inner))/(s2.e)
  Ds2e <- (Ds2e/2)*s2.e ## Chain rule for the transformation
