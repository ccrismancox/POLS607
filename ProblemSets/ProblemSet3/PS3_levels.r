...
## looking at the bias in beta_democracy
...
eff.within <- list(effects(within.l1$coefficients, 
                           vcov(within.l1),
                           differenced = FALSE),
                    effects(within.l2$coefficients,
                            vcov(within.l2),
                            differenced = FALSE),
                    effects(within.l4$coefficients, 
                            vcov(within.l4),
                            differenced = FALSE))
...
X <- data1 %>% 
  filter(year >= 1962) %>% ## 1962: we lose 2 to lags and difference
  select(starts_with("D.")) 

keep <- (1-apply(X, 1, anyNA))
X[keep==0,] <- 0
y <- X$D.y #pull the DV
Xendo <- with(X, cbind(D.l.y)) #pull the endogenous variables
...
X <- as.matrix(X)
qr(crossprod(X))$rank == ncol(X) ## check the rank



N <- length(unique(data1[data1$year >= 1962,][keep==1,]$wbcode2))
...
lagMat <- lags %>% mutate(lag_multiple(y,50:2), .by=wbcode2) ## up to 50 lags possible
lagMat <- lagMat[data1$year >= 1962,]
...
Z.list <- tapply(lagMat,
                 ...
                 \(x){
                   ...
                   as.matrix(x[,c(51:(ncol(x)-1))])) ##51+ are exogenous X and keep
...
Omega2.hat <- Reduce(`+`,
                     lapply(unique(data1$wbcode2),
                            \(i){
                              Zi <- Ze[data1[data1$year>=1962,]$wbcode2==i,]
...
X <- data2 %>% 
  filter(year >= 1963) %>% #now 1963 due to the extra lag
  select(starts_with("D.")) 
keep <- (1-apply(X, 1, anyNA))
X[keep==0,] <- 0
y <- X$D.y
...
X$D.yy1  <- NULL 
X$D.yy2  <- NULL
X$D.yy3  <- NULL #remove another year 
...
lags <- subset(data1, select=c("wbcode2", "year", "y"))
lags[is.na(lags$y),]$y <- 0
lagMat <- lags %>% mutate(lag_multiple(y,50:2), .by=wbcode2)
...
Z.list <- tapply(lagMat,
                 ...
                 \(x){
                   zout <- cbind(do.call(bdiag,
                                         apply(x[,2:49],1, #we don't want lag2 in here any more, it's now endogenous
                                               \(y){
                                                 y <- matrix(na.omit(y), nrow=1)
                                                 return(y)
                                               }
...
#### GMM lag 4#####
## lose 2 more years and year dummies
data4 <- data.ab %>% 
  arrange(wbcode2, year) %>% 
  group_by(wbcode2) %>%
  mutate(across(c(y, dem, l.y, l2.y, l3.y, l4.y, starts_with("yy")),
                ...
                .names = "D.{.col}")) %>%
  ungroup() %>% 
  mutate(across(starts_with("Ldem"), .fns=\(x){ifelse(is.na(x), 0, x)}))


X <- data4 %>% 
  filter(year >= 1965) %>%
  select(starts_with("Ldem"), starts_with("D.")) 
keep <- (1-apply(X, 1, anyNA))
X[keep==0,] <- 0
y <- X$D.y
...
N <- length(unique(data1[data1$year >= 1965,][keep==1,]$wbcode2))


lags <- subset(data1, select=c("wbcode2", "year", "y"))
lags[is.na(lags$y),]$y <- 0
lagMat <- lags %>% mutate(lag_multiple(y,50:2), .by=wbcode2)
lagMat <- lagMat[data1$year >= 1965,]
lagMat <- cbind(lagMat, X, keep)
lagMat <- lagMat %>% select(!c(year, y))
...
Omega2.hat <- Reduce(`+`,
                     lapply(unique(data1$wbcode2),
                            \(i){
                              Zi <- Ze[data1[data1$year>=1965,]$wbcode2==i,]
                              return(tcrossprod(colSums(Zi)))
                            }
                     )
)
...
ar.dat <- data.ab %>% 
  ...
  mutate(L.ehat = lag(ehat),
         L2.ehat = lag(ehat,2)) %>% 
  ungroup()
## AR(1) in the errors, -0.5 (expected)
summary(lm(ehat~L.ehat-1, data=ar.dat))
## AR(2) in the errors, 0.006 (close to 0)
...
ggplot(plot.df)+
  ...
  ylab("Change in logged GDP per capita")
...
print(xtable(out,
             align="rrddddddd",
             caption="Regression results (levels)",
             label="tab:regTab_levels"),
